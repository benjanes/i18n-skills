...
Orchestrate a comprehensive i18n readiness audit by invoking the companion audit skills and consolidating their findings. This skill does not perform analysis itself — it ensures all aspects of readiness are covered and produces a unified set of recommendations.
...
- Ensuring all audit dimensions (scope, string patterns, tone, terminology) are covered
...
- Scope only → `auditing-i18n-scope`
- String patterns / formatting only → `auditing-i18n-string-patterns`
...
|-------|---------|-----------|
| `auditing-i18n-scope` | Discover hardcoded copy, detect already-localized strings, assess scale | `i18n-pre-extraction-fixes.md` |
| `auditing-i18n-string-patterns` | Analyze string construction, pluralization, formatting, non-code content | `i18n-pre-extraction-fixes.md` + `i18n-extraction-pattern-catalog.md` |
...
|----------------|---------------|
| "Tech Stack & Configuration" or "Scope Assessment" | `auditing-i18n-scope` |
| "Pre-Extraction Fixes" (formatting issues) or extraction pattern catalog file exists | `auditing-i18n-string-patterns` |
...
1. **`auditing-i18n-scope`** (first, if missing) — produces the tech stack and string inventory that other skills consume
2. **`auditing-i18n-string-patterns`**, **`auditing-i18n-tone`**, **`auditing-i18n-terminology`** (after scope) — these are independent and can be invoked in parallel

If all skills have already run, skip to Phase 3.

### Phase 3: Consolidate

After all companion skills have completed:

1. **Read all sections** of `i18n-pre-extraction-fixes.md` written by the companion skills
...
- **Forgetting to consolidate:** The consolidation pass is the value this skill adds. Without it, the user gets four disconnected recommendation lists instead of a prioritized action plan.
